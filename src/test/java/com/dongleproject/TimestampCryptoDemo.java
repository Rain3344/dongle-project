...
public class TimestampCryptoDemo {
    ...
    // 加密时间戳 → Base64 密文
    public static String encrypt(long timestamp, String password) throws Exception {
        // 1. 生成随机 Salt 和 IV
    ...
    // 解密 Base64 密文 → 时间戳
    public static long decrypt(String encryptedBase64, String password) throws Exception {
        // 1. 解码 Base64
    ...
    public static void main(String[] args) throws Exception {
        String password = "my-custom-password-123"; // 自定义密钥种子

        // 加密示例
        long originalTimestamp = System.currentTimeMillis();
        System.out.println("原始时间戳: " + originalTimestamp);
        String encrypted = encrypt(originalTimestamp, password);
        System.out.println("加密结果: " + encrypted);

        // 解密示例
        long decryptedTimestamp = decrypt(encrypted, password);
        System.out.println("解密结果: " + decryptedTimestamp);
        System.out.println("是否匹配? " + (originalTimestamp == decryptedTimestamp));
    }

    /*public static void main(String[] args) {
        String password = "my-custom-password-aaabbb";
        String encrypted = "3nq3U9ivaq99rGptNnvY5s1nVPSCOpeQUQV6Awfarbn7VuOPEdjQXoaOComS9pz62eG3tpykWsBb";
        try {
            long decryptedTimestamp = decrypt(encrypted, password);
            System.out.println("解密结果: " + decryptedTimestamp);
        } catch (Exception e) {
